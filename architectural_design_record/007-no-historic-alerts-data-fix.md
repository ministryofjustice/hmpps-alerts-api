# ADR007: Historic Prisoner Alerts data fix will not be run in production
...
As of 11/11/2024, the DPS Alerts API is in use by all prisons and the NOMIS Alerts screen has been switched off nationwide. Prisons can no longer switch to a prisoner's previous booking and view historic alerts. As of 05/12/2024 no prison has requested the ability to view historic alerts in DPS.
...
- Historic alerts data will remain only in NOMIS and backups of the NOMIS dataset. This is in line with Syscon's request that NOMIS -> DPS migrations do not attempt to rebuild audit data from an incomplete source i.e. the booking history and NOMIS system audit columns
