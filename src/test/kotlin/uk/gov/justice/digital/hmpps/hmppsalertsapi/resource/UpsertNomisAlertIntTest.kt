...
class UpsertNomisAlertIntTest : IntegrationTestBase() {
  ...
  @Test
  fun `response contains mapping ids`() {
    val nomisAlert = nomisAlertModel()
    val nomisAlertMapping = webTestClient.upsertNomisAlert(nomisAlert)

    assertThat(nomisAlertMapping.offenderBookId).isEqualTo(nomisAlert.offenderBookId)
    assertThat(nomisAlertMapping.alertSeq).isEqualTo(nomisAlert.alertSeq)
